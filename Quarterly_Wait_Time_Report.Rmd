...
reporting_specialties <- c("Orthopedics")
...
reporting_months <- as.vector(lapply(seq.Date(from = as.Date(reporting_start), to = as.Date(reporting_end), by = "month"), function(x)format(as.Date(x), "%b")))
reporting_year <- format(as.Date(reporting_end, format="%m/%d/%Y"), "%Y") ## Create year column
...
scheduling_data <- scheduling_data_raw %>%
  ...
  filter(Appt.Month %in% reporting_months | Appt.Made.MonthYear %in% reporting_months)
...
wait_time_dist <- scheduling_data %>%
  ...
  mutate(group = case_when(Wait.Time >= 0 & Wait.Time <=7 ~ "0-7",
                           ...
                           TRUE ~ ">30")) %>%
  group_by(Campus.Specialty, Appt.Made.MonthYear, Appt.Year, Appt.Month, New.PT3, Visit.Method, group) %>%
  summarise(total = n())
...
wait_time_site <- scheduling_data %>%
  filter(Appt.Made.DateYear <= reporting_end) %>%
  filter(Wait.Time >= 0) %>%
  group_by(Campus.Specialty, Appt.Made.MonthYear, Appt.Year, Appt.Month, Campus, New.PT3) %>%
  summarise(med_wait = median(Wait.Time, na.rm = TRUE))
