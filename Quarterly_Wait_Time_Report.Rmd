...
dept_zip_tbl <- tbl(conn1, "DEPT_ZIP_CODE")
...
dept_zip_code_ref <- dept_zip_tbl %>%
  collect()
...
monthly_vol <- scheduling_data_raw %>%
  ...
  group_by(SITE, Campus.Specialty, DEPARTMENT_NAME, Appt.Year, Appt.MonNum, VISIT_METHOD) %>%
...
# require(openxlsx)
# write.xlsx(monthly_vol, "monthly_vol_7.13.23.xlsx")
```
...
vol_zip_code$dept_zip_code <- dept_zip_code_ref$`Clean Address Zip Code`[match(vol_zip_code$DEPARTMENT_NAME, dept_zip_code_ref$`Department Name`)] 
...
overall_time_to_appt_dist_function <- function(department){
  ...
  graph <- ggplot(wait_time, aes(x=factor(group, levels=c(">30","15-30","8-14","0-7")), y=perc, fill = New.PT2)) +
    ...
    theme(plot.title = element_text(hjust=0.5, face = "bold", size = 16),
            ...
            # axis.title.x = element_blank(),
            axis.line = element_line(size = 0.3, colour = "black"))+
    geom_text(aes(label= paste0(perc, " (", prettyNum(round(total), big.mark = ','),")")), hjust = -.1, vjust=.5, color="black", size=4, font.face = "bold")
  ...
  graph <- ggplot(wait_time, aes(x=reorder(Campus, med_wait), y=med_wait, group = New.PT2, fill = New.PT2)) +
    ...
    theme(plot.title = element_text(hjust=0.5, face = "bold", size = 16),
          ...
          plot.caption = element_text(size = 8, face = "bold.italic", color = "red"),
            ...
            # axis.title.x = element_blank(),
            axis.line = element_line(size = 0.3, colour = "black"))+
    geom_text(aes(label= ifelse(med_wait == 0, "", round(med_wait))), hjust = -.1, vjust=.5, color="black", size=4, font.face = "bold")
...
dept_access_takeaway <- function(department){
  ...
  dept_med_waitTime <- as.character(as.numeric(wait_time_dept$med_waitTime))
...
```{r Manual Run of Graphs, echo = FALSE, warning = FALSE, message = FALSE}

dept_list <- reporting_specialties

for(i in dept_list){
  # print(dept_quarterly_vol_function(i))
  # print(overall_time_to_appt_dist_function(i))
  # print(site_time_to_appt_function(i))
  print(dept_access_takeaway(i))
}
